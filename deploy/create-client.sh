...
# SQL Server requiere: mayúsculas + minúsculas + dígitos + especial
# Sufijo "!Aa1" garantiza los 4 requisitos sin usar # (puede romper env_file)
DB_PASSWORD="$(openssl rand -base64 32 | tr -dc 'a-zA-Z0-9' | head -c 20)!Aa1"
...
echo ""
echo -e "  ${YELLOW}1.${NC} ${RED}[DNS — HACER PRIMERO]${NC} Crear registro A en tu DNS:"
echo -e "     ${DOMAIN}  →  $(curl -4s ifconfig.me 2>/dev/null || echo '<IP-DEL-SERVIDOR>')"
echo -e "     Verificar propagación antes de continuar:"
echo -e "     nslookup ${DOMAIN} 8.8.8.8"
echo ""
echo -e "  ${YELLOW}2.${NC} Editar ${CLIENT_DIR}/.env y completar los campos CHANGE_ME:"
...
echo ""
echo -e "  ${YELLOW}3.${NC} Colocar credenciales GCS:"
echo -e "     cp /ruta/a/tu-clave.json ${CLIENT_DIR}/gcs-key.json"
echo ""
echo -e "  ${YELLOW}4.${NC} Asegurarse de que las imágenes Docker están disponibles:"
echo -e "     docker images | grep findcontrol"
echo ""
echo -e "  ${YELLOW}5.${NC} Levantar el cliente (SQL Server + Redis + Backend + Frontend):"
...
echo -e "     las migraciones automáticamente. Traefik emite el certificado SSL"
echo -e "     automáticamente al primer request HTTPS."
echo ""
echo -e "  ${YELLOW}6.${NC} Ver logs de arranque:"
echo -e "     docker compose -p ${CLIENT_NAME} logs -f backend"
echo ""
echo -e "  ${YELLOW}7.${NC} Después del primer login: deshabilitar bootstrap admin en .env:"
